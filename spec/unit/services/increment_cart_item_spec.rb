module Shop
  module Services
    RSpec.describe IncrementCartItem do
      describe '#call' do
        let(:book) do
          Models::Product.new(
        ...
        shared_examples 'raises InvalidProductIdError' do |product_id|
          it 'raises InvalidProductIdError' do
            expect { IncrementCartItem.new.call(product_id: product_id) }
              .to raise_error(IncrementCartItem::InvalidProductIdError)
          end
        end
        ...
        context 'with valid id' do
          it 'increments the Models::CartItem#quantity' do
            expect(IncrementCartItem.new.call(product_id: book.id))
              .to change { FetchCartItem.new.call(product_id: book.id).quantity }
              .from(1).to(2)
          end
        end
      end
